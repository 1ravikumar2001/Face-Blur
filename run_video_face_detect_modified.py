...
parser.add_argument('--test_device', default="cuda:0", type=str,
                    help='cuda:0 or cpu')
parser.add_argument('--video_path', default="./1.ts", type=str,
                    help='path of video')
...
while True:
    ...
    for i in range(boxes.size(0)):
        ...
        neg=cv2.bitwise_not(mask,mask = None)
        
        neg_img = cv2.bitwise_and(orig_image, orig_image, mask=neg)
        
        masked = cv2.GaussianBlur(masked, (17,17),0)
        
        orig_image = masked + neg_img
...
print("Total Time",time_end-start_time)
